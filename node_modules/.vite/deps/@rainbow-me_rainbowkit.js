...
(function(Cuer2) {
  ...
  function Root(props) {
    const { children, size: size4 = "100%", value, version: version5, ...rest } = props;
    ...
    }, [value, hasArena, props.errorCorrection, version5]);
    const cellSize = 1;
  ...
  return createConnector((config) => ({
    ...
    async getProvider() {
      if (!walletProvider) {
        ...
        const { createCoinbaseWalletSDK } = await import("./dist-4JPF4J63.js");
    ...
    async getProvider() {
      if (!walletProvider) {
        const CoinbaseWalletSDK = await (async () => {
          const { default: SDK } = await import("./dist-YSU43ZN6.js");
...
var __toESM2 = (mod3, isNodeMode, target) => {
  ...
  const to = isNodeMode || !mod3 || !mod3.__esModule ? __defProp(target, "default", { value: mod3, enumerable: true }) : target;
...
var init_sha3 = __esm(() => {
  ...
  SHA3_IOTA_H = IOTAS[0];
  SHA3_IOTA_L = IOTAS[1];
  Keccak = class Keccak2 extends Hash3 {
...
var init_encoding = __esm(() => {
  ...
  IntegerOutOfRangeError = class IntegerOutOfRangeError extends BaseError3 {
    constructor({ max, min, signed: signed2, size: size22, value }) {
      super(`Number "${value}" is not in safe ${size22 ? `${size22 * 8}-bit ${signed2 ? "signed" : "unsigned"} ` : ""}integer range ${max ? `(${min} to ${max})` : `(above ${min})`}`, { name: "IntegerOutOfRangeError" });
    }
  };
...
function numberToHex2(value_, opts = {}) {
  const { signed: signed2, size: size22 } = opts;
  ...
  const hex = `0x${(signed2 && value < 0 ? (1n << BigInt(size22 * 8)) + BigInt(value) : value).toString(16)}`;
  if (size22)
...
var hash = (value) => keccak2562(toBytes22(value));
...
function checksumAddress(address_, chainId) {
  ...
  const hash2 = keccak2562(stringToBytes(hexAddress), "bytes");
...
var init_transaction = __esm(() => {
  ...
  InvalidStorageKeySizeError = class InvalidStorageKeySizeError extends BaseError3 {
...
function assert3(value, struct, message) {
  const result = validate4(value, struct, { message });
  if (result[0]) {
    throw result[0];
  }
}
function create4(value, struct, message) {
  const result = validate4(value, struct, { coerce: true, message });
  if (result[0]) {
    throw result[0];
  } else {
    return result[1];
...
function mask(value, struct, message) {
  const result = validate4(value, struct, { coerce: true, mask: true, message });
  if (result[0]) {
    throw result[0];
  } else {
    return result[1];
...
function getSafeJson(value) {
  return create4(value, JsonStruct);
}
var jsonrpc2 = "2.0";
...
function assertTransactionEIP4844(transaction) {
  ...
  if (blobVersionedHashes) {
    ...
    for (const hash2 of blobVersionedHashes) {
      ...
      const version22 = hexToNumber2(slice3(hash2, 0, 1));
      if (size_ !== 32)
...
function serializeAuthorizationList(authorizationList) {
  ...
  for (const authorization of authorizationList) {
    ...
    serializedAuthorizationList.push([
      chainId ? toHex3(chainId) : "0x",
      contractAddress,
      nonce ? toHex3(nonce) : "0x",
...
function serializeTransactionDeposit(transaction) {
  ...
  const { sourceHash, data, from: from8, gas, isSystemTx, mint, to, value } = transaction;
  const serializedTransaction = [
    sourceHash,
    from8,
    to ?? "0x",
...
function assertTransactionDeposit(transaction) {
  const { from: from8, to } = transaction;
  if (from8 && !isAddress2(from8))
    throw new InvalidAddressError({ address: from8 });
  if (to && !isAddress2(to))
    throw new InvalidAddressError({ address: to });
}
var chainConfig = {
  ...
  return createConnector((config) => ({
    ...
    async getProvider() {
      ...
      if (!isIframe)
        return;
      if (!provider_) {
        const { default: SDK } = await import("./esm-IP7UQMNT.js");
        ...
        const SafeAppProvider = await (async () => {
          const Provider = await import("./dist-VTMFS5QB.js");
    ...
    async getProvider() {
      async function initProvider() {
        ...
        if (!optionalChains.length)
          return;
        const { EthereumProvider } = await import("./index.es-3UCYXJPQ.js");
  ...
  function addTransaction(account, chainId, transaction) {
    ...
    updateTransactions(account, chainId, (transactions) => {
      return [
        ...
        ...transactions.filter(({ hash: hash2 }) => {
  ...
  function setTransactionStatus(account, chainId, hash2, status) {
    updateTransactions(account, chainId, (transactions) => {
      return transactions.map(
        (transaction) => transaction.hash === hash2 ? { ...transaction, status } : transaction
      );
    });
  }
  async function waitForPendingTransactions(account, chainId) {
    await Promise.all(
      getTransactions(account, chainId).filter((transaction) => transaction.status === "pending").map(async (transaction) => {
        const { confirmations, hash: hash2 } = transaction;
        ...
        const requestPromise = provider.waitForTransactionReceipt({
          ...
          setTransactionStatus(account, chainId, hash2, "failed");
        });
